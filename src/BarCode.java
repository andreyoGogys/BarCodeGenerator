        ...
        try {
            ...
            for(int x = 0;x<width;x++){
                ...
                if((code.charAt(x)=='1')&(x<width)){
                    for(int y = 0;y<height;y++){
                        bufferedImage.setRGB(x+19,y,0x00000);
                    }
                }
            }
    ...
    static void code128(String code){
        ...
        if(code.length()==1)code128AorB(code);
            ...
            for(int i =0;i<code.length();i++){
                if(A_B==true)break;
                for(int j = 0;j<code128library.numbers.length;j++){
                    if(code.charAt(i)==code128library.numbers[j])
                        C=true;
                    if(
    (String.valueOf(code.charAt(i))==String.valueOf(code128library.smallSymbol[j]).toUpperCase())
                 ||
    (code.charAt(i)==code128library.smallSymbol[j])
                    )C=false;
                }
                if(C==false)A_B=true;
            }}
        }
        if(A_B==true) code128AorB(code);
        else code128C(code);
    }
